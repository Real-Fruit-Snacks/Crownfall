...
- **6 enemy archetypes** plus boss variants every 10th wave.
...
- **Commander automation** — assign per-slot commanders (auto-hire, auto-upgrade, auto-imbue, auto-skin).
...
`incremental` `idle` `tower-defense` `wave-defense` `pixel-art` `medieval` `singleplayer` `browser` `html5` `vanilla-javascript`
